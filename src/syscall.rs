...
static FD_TABLE: Mutex<BTreeMap<u64, OpenFile>> = Mutex::new(BTreeMap::new());
...
fn sys_exit(_code: u64) -> u64 {
    ...
    cleanup_fds_for_process();
    crate::scheduler::kill(pid);
    crate::scheduler::yield_now();
    0
}

fn cleanup_fds_for_process() {
    FD_TABLE.lock().clear();
}
...
fn sys_open(path_ptr: u64, path_len: u64) -> u64 {
    if path_len == 0 || path_len > 4096 { return err(EINVAL); }

    let cr3 = current_cr3();
    if !user_ptr_mapped(cr3, path_ptr, path_len) { return err(EFAULT); }
    ...
    let data = match read_file_for_fd(path_trimmed) {
        Some(d) => d,
    ...
    FD_TABLE.lock().insert(fd, OpenFile { data, offset: 0 });
    fd
}
...
fn read_file_for_fd(path: &str) -> Option<Vec<u8>> {
    ...
    let vfs_result = crate::vfs::core::with_vfs(|vfs| -> Option<Vec<u8>> {
        use crate::vfs::types::{OpenFlags, FileMode};
        let fl = OpenFlags(OpenFlags::READ);
        let fd = vfs.open(0, path_clean, fl, FileMode::default_file()).ok()?;
    ...
    if vfs_result.is_some() { return vfs_result; }
    ...
    let result = with_ext2_pub(|fs| -> Result<Vec<u8>, FsError> {
        let ino = fs.resolve_path(path_clean)?;
