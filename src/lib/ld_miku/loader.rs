...
pub fn apply_relocations(base: u64, rela_va: u64, rela_sz: u64, di: &DynInfo) {
    ...
    for i in 0..count {
        ...
        unsafe {
            match rtype {
                R_X86_64_64 => target.write_unaligned(sym_val.wrapping_add(r_addend as u64)),
                R_X86_64_GLOB_DAT | R_X86_64_JUMP_SLOT => target.write_unaligned(sym_val),
                R_X86_64_COPY => {
                    let s = (di.symtab_va + rsym as u64 * di.syment) as *const u8;
...
pub fn load_library(soname: &[u8]) {
    ...
    if fsize <= 0 || fsize > 64 * 1024 * 1024 { syscall::close(fd); return; }
    let buf = syscall::mmap(0, fsize as usize, syscall::PROT_READ | syscall::PROT_WRITE);
    if buf.is_null() { syscall::close(fd); util::println(b"[ld-miku] mmap failed for lib"); return; }
    syscall::seek(fd, 0);
    let mut done = 0usize;
    ...
    if base == 0 { util::print(b"[ld-miku] failed to map: "); util::println(name_trimmed); return; }
    register_lib(name_trimmed);
...
pub fn setup_tls(base: u64, phdrs_va: u64, phnum: u16, phent: u16) -> u64 {
    for i in 0..phnum as u64 {
        ...
        if filesz > 0 { util::memcpy(tls_mem, (base + p_vaddr) as *const u8, filesz); }
