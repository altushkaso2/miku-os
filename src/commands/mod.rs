...
pub fn execute(input: &str) {
    ...
    match cmd {
        ...
        "cd" => fs::cmd_cd(a1),
        "pwd" => fs::cmd_pwd(),
        "mkdir" => { if a1.is_empty() { println!("Usage: mkdir <name>"); } else { fs::cmd_mkdir(a1); } }
        "touch" => { if a1.is_empty() { println!("Usage: touch <name>"); } else { fs::cmd_touch(a1); } }
        "cat" => { if a1.is_empty() { println!("Usage: cat <file>"); } else { fs::cmd_cat(a1); } }
        "write" => {
            if a1.is_empty() || rest.len() <= a1.len() { println!("Usage: write <file> <text>"); }
            else { fs::cmd_write(a1, rest[a1.len()..].trim_start()); }
        }
        "stat" => { if a1.is_empty() { println!("Usage: stat <path>"); } else { fs::cmd_stat(a1); } }
        "rm" => {
        ...
        "rmdir" => { if a1.is_empty() { println!("Usage: rmdir <dir>"); } else { fs::cmd_rmdir(a1); } }
        "mv" => { if a1.is_empty() || a2.is_empty() { println!("Usage: mv <old> <new>"); } else { fs::cmd_mv(a1, a2); } }
        "ln" => {
            if a1 == "-s" {
        ...
        "readlink" => { if a1.is_empty() { println!("Usage: readlink <path>"); } else { fs::cmd_readlink(a1); } }
        "chmod" => { if a1.is_empty() || a2.is_empty() { println!("Usage: chmod <mode> <path>"); } else { fs::cmd_chmod(a1, a2); } }
        "df" => fs::cmd_df(),
