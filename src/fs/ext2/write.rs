...
impl Ext2Fs {
    ...
    fn ensure_indirect_entry(
        ...
        let existing = self.read_indirect_entry_raw(indirect_block, index)?;
        if existing != 0 {
            return Ok(existing);
        }
    ...
    fn read_indirect_entry_raw(&mut self, block_num: u32, index: u32) -> Result<u32, Ext2Error> {
    ...
    pub fn ext2_delete_file(&mut self, parent_ino: u32, name: &str) -> Result<(), Ext2Error> {
    ...
    pub fn ext2_delete_dir(&mut self, parent_ino: u32, name: &str) -> Result<(), Ext2Error> {
        let target_ino = match self.ext2_lookup_in_dir(parent_ino, name)? {
            Some(ino) => ino,
            None => return Err(Ext2Error::NotFound),
        };
    ...
    pub fn ext2_delete_recursive(&mut self, parent_ino: u32, name: &str) -> Result<u32, Ext2Error> {
        let target_ino = match self.ext2_lookup_in_dir(parent_ino, name)? {
            Some(ino) => ino,
            None => return Err(Ext2Error::NotFound),
        };
    ...
    fn free_all_blocks(&mut self, inode: &Inode) -> Result<(), Ext2Error> {
        if inode.is_symlink() && inode.is_fast_symlink() {
            return Ok(());
        }
    ...
    fn free_indirect_chain(&mut self, block: u32, depth: u32) -> Result<(), Ext2Error> {
        ...
        if depth == 1 {
            for i in 0..ptrs_per_block {
                let ptr = self.read_indirect_entry_raw(block, i)?;
    ...
    pub fn ext2_truncate(&mut self, inode_num: u32) -> Result<(), Ext2Error> {
        let mut inode = self.read_inode(inode_num)?;
    ...
    pub fn ext2_chmod(&mut self, inode_num: u32, mode: u16) -> Result<(), Ext2Error> {
        let mut inode = self.read_inode(inode_num)?;
    ...
    pub fn ext2_chown(&mut self, inode_num: u32, uid: u16, gid: u16) -> Result<(), Ext2Error> {
        let mut inode = self.read_inode(inode_num)?;
        inode.set_uid(uid);
        inode.set_gid(gid);
        let now = self.get_timestamp();
    ...
    pub fn ext2_file_size(&mut self, inode_num: u32) -> Result<u64, Ext2Error> {
        let inode = self.read_inode(inode_num)?;
        Ok(inode.size())
    }

    pub fn ext2_append_file(&mut self, inode_num: u32, data: &[u8]) -> Result<usize, Ext2Error> {
        let inode = self.read_inode(inode_num)?;
        let offset = inode.size();
        self.ext2_write_file(inode_num, data, offset)
    }

    pub fn ext2_dir_size(&mut self, dir_ino: u32) -> Result<(u32, u64), Ext2Error> {
    ...
    fn is_ext2_dir_empty(&mut self, dir_ino: u32) -> Result<bool, Ext2Error> {
        let inode = self.read_inode(dir_ino)?;
        let mut entries = [const { DirEntry::empty() }; 64];
        let count = self.read_dir(&inode, &mut entries)?;
    ...
    fn remove_dir_entry(&mut self, dir_ino: u32, name: &str) -> Result<(), Ext2Error> {
        let inode = self.read_inode(dir_ino)?;
        let bs = self.block_size as usize;
        let name_bytes = name.as_bytes();
    ...
    pub fn read_block_into(&mut self, block_num: u32, buf: &mut [u8]) -> Result<(), Ext2Error> {
